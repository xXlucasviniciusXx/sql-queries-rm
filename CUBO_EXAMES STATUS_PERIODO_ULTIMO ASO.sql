WITH UltimoASO AS (
    SELECT 
        VCONSULTASPRONT.CODPESSOA, 
        MAX(VCONSULTASPRONT.DATAASO) AS DATA_ULTIMO_ASO
    FROM VCONSULTASPRONT (NOLOCK)
    GROUP BY VCONSULTASPRONT.CODPESSOA
)
...
JOIN UltimoASO UA
    ON UA.CODPESSOA = VCONSULTASPRONT.CODPESSOA
    AND VCONSULTASPRONT.DATAASO = UA.DATA_ULTIMO_ASO  -- Filtra apenas exames da última data de ASO
